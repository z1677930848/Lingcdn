  ...
  <div>
    <n-spin :show="show" description="请稍候...">
      <n-form :label-width="100" :model="formValue" :rules="rules" ref="formRef">
        ...
        <n-divider title-placement="left">易支付</n-divider>
        <n-form-item label="网关地址" path="payYiPayGateway">
          <n-input v-model:value="formValue.payYiPayGateway" placeholder="例如：https://pay.example.com" />
          <template #feedback>末尾不用带斜杠，系统自动拼接 /submit.php</template>
        </n-form-item>

        <n-form-item label="商户ID" path="payYiPayMchId">
          <n-input v-model:value="formValue.payYiPayMchId" placeholder="" />
        </n-form-item>

        <n-form-item label="商户密钥" path="payYiPayKey">
          <n-input v-model:value="formValue.payYiPayKey" placeholder="" />
        </n-form-item>

        <n-form-item label="支付类型" path="payYiPayType">
          <n-input v-model:value="formValue.payYiPayType" placeholder="默认 alipay，可填 wxpay/alipay/..." />
          <template #feedback>对应易支付平台的 type 参数，不填写则默认 alipay</template>
        </n-form-item>
